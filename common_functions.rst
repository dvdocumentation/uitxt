...
Для размещения на экране таблицы или списка карточек используется соответвующий элемент контейнера ("Таблица" или "Список карточек") с переменной в качестве значения и нужно в коде (например в onStart экрана) определить эту переменную.

Для списка карточек  переменная будет иметь структуру:

{ "customcards": "layout": <контейнер>, "cardsdata":[{данные по строке 1},{данные по строке 2},{данные по строке N}]}, опионально может иметь объект "options"

Для таблицы:

{ "customtable": "layout":<контейнер>, "tabledata":[{данные по строке 1},{данные по строке 2},{данные по строке N}]}, опионально может иметь объект "options"

В качестве контейнера можно укать любой контейнер с люым составом элементов (*не все активные элементы экрана можно задействовать в списках).  
...
Контейнер (макет карточки или строки таблицы) удобно разработать в конструкторе, на закладке Контейнеры, присвоив уникальную переменную в качестве идентификатора такого контейнера. Тогда в переменных списка можно обратиться к этому конейнеру через ^, например вот код для создания списка карточек с двумя карточками
...
Также отдельно можно указать key – ключ, возвращаемый при нажатии в специальную переменную

Контейнер также можно разработать в «редакторе» и перенести через буфер обмена в код переменной - т.е. вставить в качестве layout JSON-объект LinearLayout
...
В кастомных списках можно делать произвольный макет каждого элемента (через _layout), в том числе и раскраску, но это затратная технология (на больших списках и медленных устройствах сказывается на быстродействии). Поэтому добавлена просто «раскраска»
...
Таблицы (простые). Работа с таблицами и план-факт
-----------------------------------------

.. note:: Для мобильной платформы рекомендуется использовать customtable  или customcards вместо обычной таблицы. Начиная с релиза 10.35 можно в контейнере использвоать padding и обводку, что даст визуальное сходство с таблицей, при гораздо больших возможностях и гораздо лучшей производительности
...
Главное меню, откуда запускаются процессы, по умолчанию имеет вид списка процессов. Прочессы к галочкой "Не отображать в меню" в таком меню не отображаются, остальные просто запускаются. Поддерживается работа с клавиатурой на уровне списка. 

Также поддерживается перехват клавиатуры для программирования клавиш.
Также можно переопределить меню в разделе Начальный экран. Можно выбрать тип меню "Процесс" и указат в "Процесс для меню" имя процесса, первый экран которого станет отображаться в качетсве меню конфигурации. Таким образом можно полностью нарисовать свое меню, использовав возможности контейнера. За запуска процессов в таком случае разработчик отвечает уже самостоятельно.

Также доступен тип меню "Плитки" (см. далее), но начаиная с релиза 12.15 рекомендуется использовать меню-экран, так как у него больше возможмностей.
