...
В самостоятельном режиме источником данных могут служить либо запрос к SQL либо GET-запрос. Для того чтобы оформить поля в строке запроса указывается "адаптер" - справоник в котором прописывается оформление карточки, картинка, ключ. 
...
Корзина - это организация списка в виде карточек (не в виде таблицы) с обработкой изменения количества и удаления, как в приложениях интернет-магазинов. Корзину можно использовать для заказов, Scan&Go розницы и много чего еще. Ввиду ограничений ОС, в контейнерах корзина не доступна - она может занимать толкьо весь экран. Поэтому ее следует размесить на экране без контейра.
