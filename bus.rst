...
.. note:: Данная функциональность работает начиная с версии приложения 12.12.05
...
Адресация до конкретного получателя
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Адресация до внутреннего механизма платформы задается в параметре **recipient**. Это означает, что обраотка будет происходить не в обычном обработчике onSimpleBusMessage, а некий механизм получит сообщение, произведет действия с ним и возможно отправит ответ.

На текущий момент доступны получатели:

**feed** - команда feed от СУБД Pelican (настройки Pelican, такие как текущая БД, должны быть заданы в конфигурации). Получив команду feed СУБД выполняет необходимые действия (изменения или выборки данных), готовит ответ и отсылает его отправителю в ответном сообщении. Таким образом моно выполнять манипуляции с СУБД из внешней системы без кода в обрбаотчиках.
